...
class Gallerytenant extends REST_Controller {

    function __construct() {
        // Construct the parent class
        parent::__construct();
        $this->load->model('Gallerytenant_model');
    }
    ...
    public function listgallery() {
        ...
        $listgallery = $this->Gallerytenant_model->listgallery($pagenumber, $pagesize, $idgallery, $idtenant, $title, $avatar);
...
///////////////////////////////////////////////////////////////////////////////////////
    public function insert_gallery() { //INSERT
        ...
        $insert_gallery = $this->Gallerytenant_model->insert_gallery($idtenant, $title, $avatar);
        if ($insert_gallery > 0) {
    ...
    public function update_gallery() { //UPDATE
        ...
        $title = addslashes($this->post('title'));
        $avatar = $this->post('avatar');
        $update_gallery = $this->Gallerytenant_model->update_gallery($idgallery, $title, $avatar);
        if ($update_gallery > 0) {
            $this->set_response(array(['message' => 'success']), REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
        } else {
            $this->not_found();
    ...
    public function delete_gallery() { //DELETE
        $idgallery= $this->post('idgallery');
        $delete_gallery = $this->Gallerytenant_model->delete_gallery($idgallery);
        if ($delete_gallery > 0) {
            $this->set_response(array(['message' => 'success']), REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
        } else {
            $this->not_found();
